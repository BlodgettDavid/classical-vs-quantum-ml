...
# Phase 3: Quantum SVM benchmark on breast cancer dataset (no PCA, config-driven)
...
# -------------------------------
# 2. Train/test split + scaling (no PCA)
# -------------------------------
...
if feature_map_name == "ZZFeatureMap":
    feature_map = ZZFeatureMap(
        feature_dimension=X_train.shape[1],  # full 30 features
        reps=cfg["qsvm"]["reps"],
...
metrics = evaluate_quantum_model(
    ...
    label="QSVM_BreastCancer",
    feature_map=feature_map,
    backend=cfg["qsvm"]["backend"]
)

# Add reproducibility + quantum hyperparameters
...
metrics["pca_components"] = 0  # explicitly no PCA
...
# -------------------------------
# 5. Visualization (optional, very slow)
# -------------------------------
...
plot_qsvm_decision_boundary(
    ...
    X_test,
    y_test,
    title=f"QSVM Breast Cancer ({metrics['dataset']}, {feature_map_name})",
    filename=plot_filename,
    do_pca=True,   # Breast Cancer dataset is high-dimensional → must reduce to 2D for visualization
