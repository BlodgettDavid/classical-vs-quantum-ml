...
# Phase 3: Quantum SVM benchmark on PCA-reduced breast cancer dataset
...
metrics = {
    "model": f"QSVM_BreastCancer_PCA_{n_components}c",
...
# 8. Visualize
# -------------------------------
plot_filename = f"QSVM_BreastCancer_PCA_{feature_map_type}_{timestamp}.png"
...
plot_qsvm_decision_boundary(
    ...
    X_test,
    y_test,
    title=f"QSVM Breast Cancer PCA ({n_components} comps, {feature_map_type} FeatureMap)",
    filename=plot_path
)
...
plot_quantum_kernel_matrix(
    kernel_matrix,
    title=f"Quantum Kernel Matrix PCA ({feature_map_type} FeatureMap)",
    filename=f"QSVM_BreastCancer_PCA_{feature_map_type}_KernelMatrix_{timestamp}.png"
)
